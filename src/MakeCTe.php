...
class MakeCTe
{
    ...
    public function monta()
    {
        ...
        if (isset($this->ICMS)) {
            $this->dom->appChild($this->imp, $this->ICMS, 'Falta tag "imp"');
        }
        if (isset($this->vTotTrib)) {
            $this->dom->appChild($this->imp, $this->vTotTrib, 'Falta tag "imp"');
        }
        ...
        if ($this->tpCTe == 1 and !empty($this->infCteComp)) { // Caso seja um CTe tipo complemento de valores
            ...
            if ($this->schema == 'PL_CTe_400_RTC') {
                foreach ($this->infDCe as $infDCe) {
                    $this->dom->appChild($this->infDoc, $infDCe, 'Falta tag "infDCe"');
                }
            }
    ...
    public function tagicms($std)
    {
        ...
        $this->dom->appChild($tagICMS, $icms);
    ...
    public function taginfDCe($std)
    {
        ...
        $std = $this->equilizeParameters($std, $possible);
        $ident = '# <infDCe> - ';
        $infDCe = $this->dom->createElement('infDCe');
        $this->dom->addChild(
            $infDCe,
            'chave',
            $std->chave,
            true,
            $ident . 'Chave de acesso da DCe'
        );
        $this->infDCe[] = $infDCe;
        return $infDCe;
    }
     ...
     * @return \DOMElement
     */
    public function tagCompraGov($std)
    {
        $possible = ['tpEnteGov', 'pRedutor'];
        $std = $this->equilizeParameters($std, $possible);
        $identificador = '# <gCompraGov> - ';
